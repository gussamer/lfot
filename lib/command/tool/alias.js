...
class Alias extends Command{
    ...
    doCommand(args){ 
      this.filename = ''
      if(args.indexOf('-f')!=-1){
        this.filename = this.args[this.args.indexOf('-f')+1]
      }
      ...
      this.overrides.forEach(override => {
        ...
        this.commAlias += 'alias '+override.alias+'="sfdx '+override.commid+'"'
      ...
      this.comms.forEach(comm => {
        if(this.overrideIdList.indexOf(comm.id)==-1){
          ...
          if(this.commAlias!=='') this.commAlias += "\n"
          this.commAlias += 'alias '+this.alias+'="sfdx '+comm.id+'"'
        }
      })
      if(this.filename){
        fs.writeFileSync(this.filename,this.commAlias)
      }else{
