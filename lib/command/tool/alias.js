...
class Alias extends Command{
    constructor(name,lfot){
        super('tool/alias',lfot)
        this.commAlias = []
        this.aliasList = []
        this.prettyList = []
        this.overrideIdList = []
        this.overrides = [
          //{alias:"fpc",commid:"project:generate"}
        ]
    }
    run(args){ 
      args = args || []
      ...
      this.commsJSON = child.execSync('sf commands --hidden --json')
      this.comms = JSON.parse(this.commsJSON)
      if(args.indexOf('-n')==-1){
        this.overrides.forEach(override => {
          this.aliasList.push(override.alias)
          this.commAlias.push('alias '+override.alias+'="sf '+override.commid+'"')
          this.prettyList.push('  '+override.alias+' = sf '+override.commid)
      ...
      this.comms.forEach(comm => {
        if(this.overrideIdList.indexOf(comm.id)==-1){
          ...
          this.commAlias.push('alias '+this.alias+'="sf '+comm.id+'"')
          this.prettyList.push('  '+this.alias+' = sf '+comm.id)
        }
      })
      ...
      if(args.indexOf('-p')==-1){
        if(this.filename){
          ...
          this.log.it('[!] wrote aliases to '+this.filename)
        }else{
