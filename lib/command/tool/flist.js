...
module.exports = class Flist extends Command{    
  constructor(name,lfot){
    super(name || 'tool/flist',lfot)
    this.schemaCommand = 'sf force schema sobject describe'    
    this.maxBufferMBs = this.lfot.settings.maxBufferMBs || 10
  }
  run(args){
    ...
    exec(comm,{maxBuffer: (1024 * 1024 * this.maxBufferMBs)},(err,stdout,stderr) => {
