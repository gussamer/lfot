...
module.exports = class Smanifest {
  constructor(argments, lfot) {
    this.lfot = lfot || {}
    this.settings = this.lfot.settings || {}
    this.apiVersion = this.settings.defaultMetadataAPIVersion
    this.apiVersionCommand = 'sf org display --json'
    this.name = ''
    this.description = ''
    this.describeCommand = 'sf org list metadata-types --json'
    this.args = argments || []
    ...
    if(this.args.indexOf('-m') != -1) {
      this.settings.maxBufferMBs = this.args[this.args.indexOf('-m') + 1]
    }
    this.settings.splitByTypeFolder = ''
    if(this.args.indexOf('-t') != -1) {
      this.settings.splitByTypeFolder = this.args[this.args.indexOf('-t') + 1]
    }
    this.settings.outputFileName = ''
    if(this.args.indexOf('-f') != -1) {
      this.settings.outputFileName = this.args[this.args.indexOf('-f') + 1]
    }
    this.stypes = []
    this.stypeNames = []
  ...
  getStypes() {
    ...
    this.apiVersion = JSON.parse(child.execSync(this.apiVersionCommand)).result.apiVersion || this.apiVersion
    ...
    if(this.isResponseGood(resp)) {
      resp.result.metadataObjects.forEach(type => {
        if(this.includeType(type)) {
          this.stypes.push(new Stype(type, this.filterNamespaced, this.lfot))
  ...
  writeXML() {
    ...
    if(this.settings.splitByTypeFolder != ''){
      let xmlFiles = {}
      let xmlString = ''
      this.stypes.forEach(type => {
        xmlString = this.packageXMLPrefix
        xmlString += type.getXML()
        xmlString += this.packageXMLSuffix
        xmlFiles[type.name] = xmlString
        let outfile = this.settings.splitByTypeFolder+'/'+type.name+'.xml'
      ...
      if(this.consoleOut) {
        ...
        if(this.settings.outputFileName != '') outfile = this.settings.outputFileName
